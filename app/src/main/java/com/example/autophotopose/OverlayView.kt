...
class OverlayView
    ...
    constructor(
        ...
        /**
         * Calculates scale factor and offsets to fit the image within the view
         * while maintaining aspect ratio (center-crop style).
         */
        private fun calculateScaleAndOffset() {
            ...
            if (viewWidth <= 0 || viewHeight <= 0 || imageWidth <= 0 || imageHeight <= 0) {
                Log.w(TAG, "Invalid dimensions for scaling: view=${viewWidth}x$viewHeight, image=${imageWidth}x$imageHeight")
                scaleFactor = 1f
                offsetX = 0f
                offsetY = 0f
                return
            ...
            if (viewAspectRatio > imageAspectRatio) {
                // View is wider than image: scale by height, center horizontally
                scaleFactor = viewHeight / imageHeight
                offsetX = (viewWidth - imageWidth * scaleFactor) / 2f
                offsetY = 0f
            } else {
                // View is taller than image: scale by width, center vertically
